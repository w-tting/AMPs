...
def plot_comparison(data_pairs):
    num_plots = len(data_pairs)
    processed_data = []
    max_densities = []
    for seqs, value, name in data_pairs:
        res = calc_hmoment(seqs)
        ylabel='Global Charge' 
        processed_data.append((value, res, name)) 
        H, _, _ = np.histogram2d(value, res, bins=10, density=True)
        max_densities.append(np.max(H))  
    #Determine a unified vmax
    ...
    image = None 
    #plot
    for i, (value, res, name) in enumerate(processed_data):
        ax = axes[i]     
        _, _, _, image = ax.hist2d(value, res, bins=20,  cmap='Blues', density=True, vmin=0, vmax=vmax)        
        ax.set_xlabel("Log MIC", fontsize=32)
        if i == 0: 
        ...
        #change with ylabel
        ax.set_ylim(-4, 18)
        ax.set_yticks(np.arange(0, 18, 8))
        
        ax.tick_params(axis='both', width=1.5, labelsize=28)
    cbar = fig.colorbar(image, ax=axes, fraction=0.15, pad=0.02)

#     image.set_clim(0, 0.2)
...
if __name__ == '__main__':          
    ...
    #property hist
    base_df=pd.read_csv('../data/output/Base_gen.csv',index_col=False)
    rm_df=pd.read_csv('../data/output/RM_gen.csv',index_col=False)
    sm_df=pd.read_csv('../data/output/SM_gen.csv',index_col=False)
    input_data = [(base_df['sequence'].tolist(), base_df['value'], "Base"),(rm_df['sequence'].tolist(), rm_df['value'], "RM"),(sm_df['sequence'].tolist(), sm_df['value'], "SM")]
    plot_comparison(input_data)          